/* Service worker for TheGames
   - Caches index.html and other assets relative to the SW's location
   - Works when the app is served from a repo subpath (GitHub Pages project pages)
*/
...
async function cacheIndexAndAssets(cache) {
  try {
    ...
    // collect local src/href values from index.html
    const re = /(?:src|href)="([^"#?]+)(?:\?[^\"]*)?"/g;
    let m;
